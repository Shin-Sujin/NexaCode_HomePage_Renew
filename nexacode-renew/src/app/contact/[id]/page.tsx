...
export default function ContactAnswerPage({
  params,
}: {
  params: { id: string };
}) {
  // const answerData = fetchInquiryDetail.find((d) => d.id === Number(params.id));
  ...
  const [maskedResponse, setMaskedResponse] = useState("");
  ...
  useEffect(() => {
    if (inquiryDetail && inquiryDetail.response) {
      const lines = inquiryDetail.response.split("\n");
      const firstLine = lines[0];
      const maskedLines = lines.slice(1).map((line) => line.replace(/./g, "*"));
      setMaskedResponse([firstLine, ...maskedLines].join("\n"));
    }
  }, [inquiryDetail]);

  if (loading) return <div>로딩 중...</div>;
  if (error) return <div>{error}</div>;
  if (!inquiryDetail) return <div>존재하지 않는 글입니다.</div>;
      ...
      <main className="max-w-5xl mx-auto px-4 pt-48 max-md:w-full max-md:pb-20">
        ...
        <form className="w-full pb-20">
          ...
          <div className="pt-10 pb-10">
            ...
            <Checkbox.Group
              name="area"
              value={[inquiryDetail.developmentAreaId]}
              disabled
            >
          ...
          <div className="mb-10">
            ...
            <Checkbox.Group
              name="step"
              value={[inquiryDetail.preparationStageId]}
              disabled
            >
              <Space direction="vertical" size={12}>
                <Checkbox value={1} className="text-base  text-[#313131]">
          ...
          <div className="mb-10">
            ...
            <Radio.Group
              name="pageCount"
              value={inquiryDetail.workPageId}
              disabled
            >
              <Space direction="vertical" size={12}>
          ...
          <div className="mb-10">
            ...
            <TextArea
              ...
              value={inquiryDetail.serviceSummary}
              disabled
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="mb-10">
              ...
              <Input
                ...
                required
                className="h-[40px]"
                value={inquiryDetail.companyName}
                disabled
              />
            </div>
